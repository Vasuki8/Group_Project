...
app.get("/api/movies/new", (req, res) => {
  res.render("movie-form"); // Renders the movie-form.handlebars file in the views directory
});
...
app.get('/api/movies', async (req, res) => {
    try {
        ...
        // Render the 'movies-list' Handlebars template and pass the movie data
...
app.put("/api/movies/:movie_id", async function (req, res) {
  try {
    ...
    existingMovie.num_mflix_comments =
      req.body.num_mflix_comments || existingMovie.num_mflix_comments;
    ...
    existingMovie.lastupdated =
      req.body.lastupdated || existingMovie.lastupdated;
